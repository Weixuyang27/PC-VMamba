...
from models.Mamba.PCM import MultiScan
...
class VSSM(nn.Module):
    def __init__(
        ...
        decoder_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths_decoder))]  # stochastic depth decay rule
        _NORMLAYERS = dict(
            ln=nn.LayerNorm,
            bn=nn.BatchNorm2d,
        )
        ...
        for i_layer in range(self.num_layers):
            self.layers_up.append(self._make_layer_up(
                ...
                ssm_simple_init=ssm_simple_init,
                # =================
                directions=None if decoder_directions is None else decoder_directions[sum(depths_decoder[:i_layer]):sum(depths_decoder[:i_layer + 1])]
            ))
...
@register_model
def PCViM(*args, in_chans=3, num_classes=1, dims=[96, 192, 384, 768],
          depths=[2, 2, 2, 2], depths_decoder=[2, 2, 2, 1], dims_decoder=[768, 384, 192, 96],
